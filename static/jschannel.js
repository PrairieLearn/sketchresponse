    ...
    build: function (cfg) {
      ...
      var setTransactionTimeout = function (transId, timeout, method) {
        return window.setTimeout(function () {
          if (outTbl[transId]) {
            ...
            var msg =
              "timeout (" + timeout + "ms) exceeded on method '" + method + "'";
      ...
      var onMessage = function (origin, method, m) {
        ...
        if (m.id && method) {
          ...
          if (
            !outTbl[m.id] ||
            !outTbl[m.id].callbacks ||
            !outTbl[m.id].callbacks[m.callback]
          ) {
            debug(
              "ignoring invalid callback, id:" + m.id + " (" + m.callback + ")",
            );
          } else {
            // XXX: what if client code raises an exception here?
