...
from ..grader_lib import GradeableFunction, LineSegment, PolyLine
...
def grader(pl, pt, h, r, ls, cwm, ccwm):
    ...
    if pl.get_polyline_count() > 0:
        ...
        for segment in beam.segments:
            if beam.check_segment_startpoint(segment, [-3, 2]) and not beam.check_segment_endpoint(
                segment, [-3, -1]
            ):
                return False, "Check beam segment connected to A"

            if beam.check_segment_startpoint(segment, [-3, -1]) and not beam.check_segment_endpoint(
                segment, [2, -1]
            ):
